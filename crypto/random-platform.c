...
{
#ifdef _WIN32
    /* BCryptGenRandom with BCRYPT_USE_SYSTEM_PREFERRED_RNG needs no provider
     * handle and is available to packaged/UWP applications. */
#else
# ifdef CONFIG_GETRANDOM
                         ...
                         Error **errp)
{
    ...
    if (buflen > ULONG_MAX) {
        error_setg(errp, "Random byte request is too large");
        return -1;
    }
    status = BCryptGenRandom(NULL, buf, (ULONG)buflen,
                             BCRYPT_USE_SYSTEM_PREFERRED_RNG);
    if (!BCRYPT_SUCCESS(status)) {
        error_setg(errp, "BCryptGenRandom failed with status 0x%08lx",
